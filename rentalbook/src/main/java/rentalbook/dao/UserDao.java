...
public class UserDao {
	...
	//PK로 조회를 해야되는 상황이 많을거임. 그 작업을 메서드화
	// 리턴 벨류가 닉네임 혹은 패스워드 같은 단일 밸류면 그 타입을 설정하면 됨
	// 지금 만들고자 하는 목적은 모든 컬럼의 정보가 조회되길 원함
	// 전부 다 담을 객체를 설계해서 리턴하는 데 그런 용도의 객체를 DTO(Transfer Object) 혹은 VO(Value Object)라고 부른다.
	...
	public boolean updateNickname(User oldUser) throws SQLException {		
		...
		PreparedStatement stmt = conn.prepareStatement("UPDATE USERS SET NICKNAME=? WHERE USER_ID=?");
		stmt.setString(1, oldUser.nickname());
