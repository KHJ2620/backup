...
<body>
	...
	<p>request 객체의 클라이언트 요청 정보를 담고 있는 굉장히 중요한 객체이다. 이미 데이터 입력값 얻어낼 때
		getParameter 메서드를 사용해보았는데, 이것 말고도 굉장히 많은 메서드들을 가지고 있다. 중요하고 자주 사용되는
		메서드들에 대해서 살펴보자.</p>
	<h3>기본 정보 메서드</h3>
	<%
	...
	<h3>헤더 관련 메서드</h3>
	<p>브라우저는 사용자가 웹 요청을 보내면, 기본적인 정보를 같이 전달한다. 이 정보값들은 요청헤더에 설정되어서 넘어가게
		전달된다. 이 값들을 활용해서 사용하고 싶다면 헤더관련 메서드를 이용하면 된다.</p>
	<%
	...
	<ul>
		...
		<li>getParamterMap : 모든 요청 파라미터를 맵 형태로 추출 ==> Map&lt;String, String[]&gt; </li>
	</ul>
	<h3>기타 유용한 메서드</h3>
	<p>
		이 외에도 세션, 쿠키, 인증 정보를 추출하는 메서드들도 존재한다. 
		이 메서드들은 로그인기능 같은 구현하면서 사용법을 익혀보자.
	</p>
