...
<h2>사용자와 웹 앱간의 상호작용</h2>
	<p>
		사용자로부터 요청이 발생하였을 때, 같이 전달된 데이터들은 request 객체를 이용해서 얻어낼 수 있고,
		이걸 토대로 요청을 분석해서 적절한 응답을 보내줄 수 있는 페이지 구축이 가능하다.
	</p>
	<p>
		웹 앱 개발을 할 때는 데이터 처리 페이지만 만들어두면 사용자가 이 웹앱을 쓰는데 있어 불편함을 느낄 수밖에 없다.
	</p>
	<p>
	...
	<p>
		추천 검색어 <a href = "./serch-result.jsp?subject=page">페이지</a>
		 <a href = "./serch-result.jsp?subject=include">인클루드</a>
	...
	<form action="./search-result.jsp">
		<p>
			입력 : <input type="text" name="subject"/> <!-- 같은 이름이면 뒤에 것은 무시 -->
	...
	<p>
		...
		입력용 태그에는 name이 설정되어야 한다. (네임에 설정한 값으로 데이터를 넘겨준다.) 
	</p>
</body>
</html>